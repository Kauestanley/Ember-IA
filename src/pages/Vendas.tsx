...
function KpiCard({ label, value, delta, icon: Icon, iconColor, iconBg, deltaLabel }: {
  ...
  const isPos = delta >= 0
  return (
    <Card className="card-glow border-white/[0.06] bg-card hover:border-white/[0.1] transition-all duration-200 hover:-translate-y-px">
      <CardContent className="p-6">
        <div className="flex items-start justify-between mb-4">
          <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground leading-tight max-w-[120px]">
            {label}
          </p>
        ...
        <p className="text-4xl font-black tracking-tight number-gradient mb-2">{value}</p>
        <div className="flex items-center gap-1.5">
          {isPos
            ? <ArrowUpRight className="h-3.5 w-3.5 text-emerald-400" />
            : <ArrowDownRight className="h-3.5 w-3.5 text-rose-400" />
          }
          <span className={cn('text-xs font-semibold', isPos ? 'text-emerald-400' : 'text-rose-400')}>
            {Math.abs(delta)}%
          </span>
...
const CustomTooltip = ({ active, payload, label }: {
  ...
  return (
    <div className="rounded-xl border border-white/[0.08] bg-[hsl(222,60%,4%)] p-3 shadow-2xl text-sm min-w-[160px] backdrop-blur-sm">
...
export function Vendas() {
  ...
  if (loading || !m) {
    return (
      <div className="flex items-center justify-center h-full min-h-[60vh] gap-3 text-muted-foreground">
  ...
  const kpis = [
    ...
    { label: 'Tempo Médio Contrato', value: `${m.avgContractDuration}m`, delta: 2.8, icon: Clock, iconColor: 'text-blue-400', iconBg: 'bg-blue-500/10' },
  ...
  return (
    <div className="p-8 space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card className="card-glow border-white/[0.06] bg-gradient-to-br from-indigo-500/[0.08] to-purple-500/[0.04]">
          <CardContent className="p-6">
            <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-2">Ratio LTV:CAC</p>
            <p className="text-5xl font-black tracking-tight number-gradient">{ratio.toFixed(1)}<span className="text-3xl">x</span></p>
            <p className="mt-2 text-xs">
              {ratio >= 3
                ? <span className="text-emerald-400 font-medium">Excelente — meta: &gt; 3x ✓</span>
                : <span className="text-amber-400 font-medium">Abaixo do ideal — meta: &gt; 3x</span>
              }
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="card-glow border-white/[0.06]">
          <CardContent className="p-6">
            <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-2">Gastos em Marketing</p>
            <p className="text-4xl font-black tracking-tight number-gradient">{formatCurrency(m.marketingSpend)}</p>
            <p className="mt-2 text-xs text-muted-foreground">
              Ads: <span className="text-foreground font-semibold">{formatCurrency(m.adSpend)}</span>
              {' · '}Orgânico: <span className="text-foreground font-semibold">{formatCurrency(m.marketingSpend - m.adSpend)}</span>
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="card-glow border-white/[0.06]">
          <CardContent className="p-6">
            <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-2">Novos Clientes (mês)</p>
            <p className="text-5xl font-black tracking-tight number-gradient">{m.newClientsThisMonth}</p>
            <div className="mt-2 flex items-center gap-1.5 text-xs">
              <ArrowUpRight className="h-3.5 w-3.5 text-emerald-400" />
              <span className="text-emerald-400 font-semibold">{m.revenueGrowthMoM}%</span>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={240}>
              <AreaChart data={monthlyRevenue} margin={{ top: 5, right: 10, left: 10, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="faturamento" name="MRR" stroke="#6366f1" strokeWidth={2.5} fill="url(#mrrGrad)" dot={false} activeDot={{ r: 5, fill: '#6366f1', strokeWidth: 0 }} />
              </AreaChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <Card className="card-glow border-white/[0.06]">
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={160}>
              <PieChart>
                <Pie data={salesBySegment} cx="50%" cy="50%" innerRadius={42} outerRadius={68} paddingAngle={3} dataKey="value">
                  {salesBySegment.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} strokeWidth={0} />)}
                </Pie>
                <Tooltip
                  formatter={(value) => [`${value}%`, '']}
                  contentStyle={{ background: 'hsl(222,60%,4%)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '10px', fontSize: '12px' }}
                />
              </PieChart>
            </ResponsiveContainer>
            <div className="mt-1 grid grid-cols-2 gap-1.5">
              {salesBySegment.map((s, i) => (
                <div key={s.segment} className="flex items-center gap-1.5 text-xs">
                  ...
                  <span className="ml-auto font-semibold text-foreground/80">{s.value}%</span>
                </div>
              ))}
            </div>
          </CardContent>
      ...
      <div className="grid grid-cols-2 gap-4">
        <Card className="card-glow border-white/[0.06]">
          <CardHeader className="pb-2">
            <CardTitle className="text-base font-semibold">Churn vs Novos Clientes</CardTitle>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={churnTrend} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" />
                <XAxis dataKey="month" tick={{ fontSize: 11, fill: 'hsl(215,20%,50%)' }} axisLine={false} tickLine={false} />
                <YAxis yAxisId="left" tick={{ fontSize: 11, fill: 'hsl(215,20%,50%)' }} axisLine={false} tickLine={false} />
                <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 11, fill: 'hsl(215,20%,50%)' }} axisLine={false} tickLine={false} tickFormatter={v => `${v}%`} />
                <Tooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{ fontSize: '12px', color: 'hsl(215,20%,50%)' }} />
                <Line yAxisId="right" type="monotone" dataKey="churn" name="Churn" stroke="#f43f5e" strokeWidth={2} dot={{ fill: '#f43f5e', r: 3, strokeWidth: 0 }} />
                <Line yAxisId="left" type="monotone" dataKey="newClients" name="Novos Clientes" stroke="#10b981" strokeWidth={2} dot={{ fill: '#10b981', r: 3, strokeWidth: 0 }} />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <Card className="card-glow border-white/[0.06]">
          ...
          <CardContent>
                  ...
                  <div key={stage.stage}>
                    <div className="flex items-center justify-between mb-1.5">
                      <div className="flex items-center gap-2">
                        <div className="flex h-5 w-5 items-center justify-center rounded-md text-[9px] font-bold bg-white/[0.06] text-muted-foreground">
                          {i + 1}
                        </div>
                        <span className="text-sm font-medium">{stage.stage}</span>
                        {i > 0 && (
                          <Badge
                            variant={convRate >= 50 ? 'success' : convRate >= 30 ? 'warning' : 'destructive'}
            ...
            <div className="mt-4 rounded-xl bg-white/[0.04] p-3 flex items-center justify-between border border-white/[0.05]">
